...
export class MockProductInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (request.method === 'GET' && request.url.includes('/product')) {
      ...
      return of(new HttpResponse({ status: 200, body: mockProduct})).pipe(delay(500));
    }

    if (request.method === 'POST' && request.url.includes('/cart/add')) {
      const requestBody: { productId: number, quantity: number } = request.body

      LocalStorageHandler.addProductToShoppingCart(requestBody.productId, requestBody.quantity);

      const mockResponse = {
        status: 'success',
        message: 'Product ' + requestBody.productId + ' added to cart. Quantity: ' + requestBody.quantity
      };
